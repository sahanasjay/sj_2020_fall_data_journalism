...
# The names of the colums are all messed up! State is the best one, and it's capitazlized. The other two have periods and numbers and are extremely lengthy so I can't really tell what the numbers in them even represent. Beyond that, I think the data is reading the numbers in as words, based on the glimpse.Also, some N/As, a loooot of missing values with "*" instead of a number. 
# From the video I see the asterisk means N/A and also that the words in the two numeric columns are what made R desicde they are words 
...
opioid_scrape_clean <- opioid_scrape %>%
  clean_names() %>%
  rename(opioid_death_rate = opioid_involved_overdose_deaths_100_000_persons1_2018) %>%
  rename(opioid_prescriptions_rate = opioid_prescriptions_100_persons2_2018)
