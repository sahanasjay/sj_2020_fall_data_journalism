...
Sahana notes: to fix error in code, best way is google. Also, if you don't load tidyverse, some things wont work (like the read_csv below)
...
```{r}
arcos_md_baltimore_city <- read_csv("data/arcos-md-baltimore-city-24510-pharmacy.csv")
```
...
```{r}
arcos_md_baltimore_city_pharmacies <- arcos_md_baltimore_city %>%
  select(buyer_name)
...
```{r}
arcos_md_baltimore_city_trimmed <- arcos_md_baltimore_city %>%
  select(buyer_name, buyer_dea_no, buyer_city, total_dosage_unit, total_records)
...
MY ANSWER: Newcare home health services!
```{r}
arcos_md_baltimore_city_sorted <- arcos_md_baltimore_city_trimmed %>%
  arrange(desc(total_dosage_unit))
...
```{r}
# Answer to question 1: Giant pharmacy 166 got the most shipments between 2006 and 2014, at 10,365 shipments
arcos_md_baltimore_city_trimmed_sorted <- arcos_md_baltimore_city %>%
  select(buyer_name, buyer_dea_no, buyer_city, total_dosage_unit, total_records) %>%
...
```{r}
# Answer to question 2: The buyer who got the smallest number of pills was Ohinsky, AlAN E, MD., and they received 4 pills. However, that's a doctor; the pharmacy that received the fewest doeses looks to be the NIA research pharmacy at 100 pills
arcos_md_baltimore_city_trimmed_sorted <- arcos_md_baltimore_city %>%
  select(buyer_name, buyer_dea_no, buyer_city, total_dosage_unit, total_records) %>%
  arrange(total_dosage_unit)
```
