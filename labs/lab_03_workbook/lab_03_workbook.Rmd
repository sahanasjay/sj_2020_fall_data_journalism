...
# A: Prince George's County had the highest case rate, at ~29 cases pero 1000 (29.27, to be exact).
...
# A: Bath county had the lowest case rate on Aug. 30, at .91 cases per thousand. Greensville county had the highest, at 60.5 cases per thousand. 
...
# A: 2 areas: Hancock county, Georgia, and Galax city, Virginia.
...
# A: 3 areas: Galax city, Emporia city, and Northampton. 
...
covid_county_death_jun_aug_pct_change <- covid_county_monthly_death_rate %>%
...
```{r}
covid_county_case_may_aug_pct_change <- covid_county_monthly_case_rate %>%
...
covid_county_case_may_aug_pct_change

# A: Jones county, Texas saw the smallest percent change in cumulative case rate between May and August
```
...
# Q: Which county in Oregon had the largest percent jump in death rates between May and August? 
covid_county_oregon_death_may_aug_pct_change <- covid_county_monthly_death_rate %>%
  select(county, state, d_2020_05_30, d_2020_08_30)%>%
  filter(state=="Oregon")%>%
  mutate(may_aug_pct_change=(d_2020_08_30 - d_2020_05_30)/d_2020_05_30)%>%
  arrange(desc(may_aug_pct_change))

covid_county_oregon_death_may_aug_pct_change

# A: Umatilla, Oregon had the largest percent jump in death rates between May and August
```
