...
# Write the answer in a comment
# Q: Which state has the most cases per counties in our data? How many?
# A: Arizona, about 13446
```
...
```{r}
average_county_deaths_real <- covid_county%>%
  group_by(state)%>%
  summarise(average_county_deaths = mean(deaths))%>%
  arrange(desc(average_county_deaths))

average_county_deaths_real

#A: New Jersey had the most, Alaska had the least  
