...
```{r}
state_case_per_thousand <- state_covid_population%>%
  select(state, total_cases, population)%>%
  mutate(cases_per_thousand = (total_cases/population)*1000 )%>%
  arrange(desc(cases_per_thousand))
state_case_per_thousand

#A: Lousiana had the highest number of cases per thousand 

```
```{r}
state_deaths_per_thousand <- state_covid_population%>%
  select(state, total_deaths, population)%>%
  mutate(deaths_per_thousand = (total_deaths/population)*1000 )%>%
  arrange(deaths_per_thousand)
state_deaths_per_thousand

#A: Hawaii had the lowest number of deaths per thousand 
```
-30-
